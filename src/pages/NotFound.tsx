...
const NotFound: React.FC<NotFoundProps> = ({ lang = 'vi' }) => {
  ...
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4 animate-in fade-in duration-500">
      ...
      <div className="text-6xl md:text-8xl font-black text-slate-900 dark:text-white mb-2 tracking-tighter">
        404
      </div>
      ...
      <p className="text-slate-600 dark:text-slate-300/85 max-w-md mb-10 text-sm md:text-base">
        {isVi
      ...
      <div className="flex flex-wrap gap-3 justify-center">
        <Link
          to="/"
          className="flex items-center gap-2 bg-primary text-slate-900 dark:text-white font-bold px-5 py-2.5 rounded-lg text-sm hover:bg-blue-500 transition-colors"
        >
          <Home size={15} />
          {isVi ? 'Trang chủ' : 'Home'}
        </Link>
        ...
        <Link
          to="/academy"
          className="flex items-center gap-2 border border-white/15 text-slate-200 font-bold px-5 py-2.5 rounded-lg text-sm hover:border-primary/60 hover:text-slate-900 dark:text-white transition-colors"
        >
