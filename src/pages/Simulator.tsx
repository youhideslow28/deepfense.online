...
const Simulator: React.FC<SimulatorProps> = ({ lang }) => {
  ...
  const handleReport = async () => {
    setStatus('success');
    const reward = timer <= 180 ? 10 : 5;
    const result = await claimDpfReward({
      source: 'simulator',
      activityId: `ceo-transfer-scam-${sessionId}`,
      amount: reward,
      reason: lang === 'vi' ? 'Vuot qua mo phong lua dao CEO chuyen tien' : 'Passed CEO transfer scam simulation',
      dailyLimit: 3,
      minScore: 70,
      score: timer <= 180 ? 100 : 75,
      metadata: {
        scenarioId: 'ceo-transfer-scam',
        level: 'basic',
