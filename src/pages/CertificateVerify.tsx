      ...
      <div className="relative overflow-hidden rounded-2xl border border-emerald-400/25 bg-[#07111f]/95 p-6 md:p-10 shadow-[0_24px_90px_rgba(0,0,0,0.42)] backdrop-blur-xl">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl md:text-5xl font-black uppercase leading-tight text-white" style={{ fontFamily: "var(--font-display)" }}>
              {verified ? (isVi ? 'Chứng chỉ hợp lệ' : 'Certificate Verified') : (isVi ? 'Cần mã chứng chỉ' : 'Certificate ID Required')}
            </h1>
            <p className="mt-4 max-w-2xl text-sm md:text-base leading-relaxed text-slate-300/85">
              {isVi
            ...
            >
              <input
                ...
                className="min-h-12 flex-1 rounded-lg border border-white/12 bg-black/35 px-4 text-sm font-bold text-white outline-none placeholder:text-slate-600 focus:border-emerald-300/60"
              />
              <button className="rounded-lg bg-emerald-500 px-5 py-3 text-xs font-black uppercase tracking-[0.12em] text-white hover:bg-emerald-400">
                Verify ID
              </button>
            </form>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
          <div className="rounded-xl border border-white/12 bg-black/35 p-4">
            <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 mb-2">Certificate ID</div>
            <div className="text-white font-black break-all">{activeId || 'N/A'}</div>
          </div>
          <div className="rounded-xl border border-white/12 bg-black/35 p-4">
            <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 mb-2">{isVi ? 'Khóa học' : 'Course'}</div>
            <div className="text-white font-black">DEEPFENSE BASIC</div>
          </div>
          <div className="rounded-xl border border-white/12 bg-black/35 p-4">
            <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 mb-2">{isVi ? 'Tên hiển thị' : 'Display name'}</div>
            <div className="text-white font-black">{certificateName || auth.displayName || (isVi ? 'Không công khai' : 'Not public')}</div>
          </div>
          <div className="rounded-xl border border-white/12 bg-black/35 p-4">
            <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 mb-2">{isVi ? 'Ngày cấp' : 'Issued'}</div>
            <div className="text-white font-black">{issuedDate}</div>
          </div>
        </div>

        <div className="flex flex-col sm:flex-row gap-3 mt-7">
          <button onClick={copyLink} className="inline-flex items-center justify-center gap-2 rounded-lg border border-white/12 bg-white/[0.08] px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-white hover:bg-white/12">
