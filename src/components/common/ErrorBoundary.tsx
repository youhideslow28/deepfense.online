...
class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    logger.error("React application error caught:", error);
    ...
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center py-20 font-mono text-center px-4 animate-in fade-in">
          ...
          <p className="text-slate-600 dark:text-slate-300/85 text-sm mb-8 max-w-md">
          ...
          <button 
            ...
            className="bg-primary text-slate-900 dark:text-white px-8 py-3 rounded-xl font-bold uppercase tracking-[0.12em] hover:bg-blue-500 transition-colors"
          >
            TẢI LẠI TRANG
          </button>
        </div>
