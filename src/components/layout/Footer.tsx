...
const Footer: React.FC<FooterProps> = ({ lang, siteConfig, isFamilyShell, familyAudience }) => {
  ...
  const trainingLinks = [
    ...
    { path: '/tools/scan', label: isVi ? 'Công cụ an toàn' : 'Safety Tools', icon: <ShieldCheck size={13} /> },
  ...
  const supportLinks = [
    ...
    { path: '/contact', label: isVi ? 'Liên hệ chúng tôi' : 'Contact Us', icon: <Mail size={13} /> },
  ...
  const renderLink = (item: typeof trainingLinks[number] | typeof supportLinks[number] | typeof legalLinks[number]) => {
    const className = 'group flex items-center justify-between gap-3 rounded-md px-2 py-2 text-sm text-slate-500 dark:text-slate-400 transition-colors hover:bg-white/[0.035] hover:text-blue-200';
    const content = (
      <>
        <span className="flex items-center gap-2 min-w-0 overflow-hidden">
          <span className="shrink-0 text-slate-600 transition-colors group-hover:text-blue-300">{item.icon}</span>
          <span className="truncate">{item.label}</span>
        </span>
        {'external' in item && item.external ? (
          <ExternalLink size={11} className="shrink-0 opacity-50" />
        ) : (
          <ChevronRight size={12} className="shrink-0 opacity-0 transition-all group-hover:translate-x-1 group-hover:opacity-100" />
        )}
      </>
    );
        ...
        <div className="grid grid-cols-1 gap-9 lg:grid-cols-12">
          <div className="lg:col-span-4">
            <Link to="/" className="mb-5 inline-flex items-center gap-3 group">
              ...
              <div>
                <div className="font-display text-2xl font-black tracking-tight text-slate-900 dark:text-white">
                  DEEPFENSE
                </div>
            ...
            <p className="max-w-2xl text-sm leading-7 text-slate-600 dark:text-slate-300/82 md:text-justify">
              {isVi
            ...
            <div className="mt-6 rounded-xl border border-black/10 dark:border-white/10 bg-white/[0.035] p-5">
          ...
          <div className="grid gap-8 grid-cols-2 lg:col-span-5">
            <div>
          ...
          <div className="lg:col-span-3">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 text-sm leading-relaxed text-slate-500 dark:text-slate-400">
              ...
              <a href={`mailto:${PROJECT_METADATA.email}`} className="flex items-start gap-3 text-sm leading-relaxed text-slate-500 dark:text-slate-400 transition-colors hover:text-blue-200">
        ...
        <div className="mt-10 border-t border-[#1E3A5F]/30 pt-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="font-mono text-[10px] leading-relaxed text-slate-500">
              © 2025 DEEPFENSE · {isVi ? 'Sinh viên VKU (25NS)' : 'VKU Students (25NS)'}. {isVi ? 'Bảo lưu mọi quyền.' : 'All rights reserved.'}
            </div>
