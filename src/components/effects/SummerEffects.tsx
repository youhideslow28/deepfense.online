...
const SummerEffects: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const newPetals = Array.from({ length: 40 }).map((_, i) => ({
      ...
      color: Math.random() > 0.5 ? '#ff2a00' : '#ff5500' // Äá» vÃ  Cam (mÃ u cá»§a hoa phÆ°á»£ng)
    }));
    setPetals(newPetals);

    // Generate cicadas (ve sáº§u)
    const newCicadas = Array.from({ length: 5 }).map((_, i) => ({
      id: i,
      top: 15 + Math.random() * 70, // 15% to 85% chiá»u cao
      animationDuration: 12 + Math.random() * 15, // Thá»i gian bay ngang
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
      ...
      {petals.map((petal) => (
        <div
          ...
          style={{
            ...
            borderRadius: '50% 0 50% 0', // Táº¡o hÃ¬nh dáº¡ng giá»‘ng cÃ¡nh hoa
      ...
      {cicadas.map((cicada) => (
        ...
        >
          {/* Icon con ve (SVG) xoay ngang 90 Ä‘á»™ Ä‘á»ƒ bay tá»« trÃ¡i sang pháº£i */}
